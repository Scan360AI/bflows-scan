    ...
    // --- PARTE 2: Economico ---
         ...
         try {
             ...
             const options = {
                ...
                scales: {
                     ...
                     y1: { position: 'right', title: { display: true, text: 'EBITDA %' }, grid: { drawOnChartArea: false }, suggestedMin: -100, suggestedMax: 15, ticks: { callback: function(v) { return v + '%';}} }
                 },
     ...
     if (document.getElementById('marginalityChart')) {
         try {
             ...
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Percentuale sui Ricavi (%)' }, suggestedMin: -50, suggestedMax: 100, ticks: { callback: function(v) { return v + '%';}} } },
     ...
     if (document.getElementById('profitabilityIndicesChart')) { // Rinominato da profitabilityChart
         try {
            const data = getProfitabilityIndicesData(); // Assicurati che la funzione esista in config
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Percentuale (%)' }, suggestedMin: -100, suggestedMax: 40, ticks: { callback: function(v) { return v + '%';}} } },
     ...
     if (document.getElementById('leverageChart')) {
          try {
             ...
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Percentuale (%)' }, suggestedMin: -90, suggestedMax: 0, ticks: { callback: function(v) { return v + '%';}} } },
         ...
         try {
            const data = getDebtCostData();
             const options = {
                 ...commonChartOptions,
                 scales: {
                     y: { ...commonChartOptions.scales.y, type: 'linear', position: 'left', title: { display: true, text: 'Valore (€000)' }, suggestedMin: -100, ticks: { callback: function(v) { return v.toLocaleString('it-IT'); } } },
    ...
    if (document.getElementById('workingCapitalCycleChart')) {
         try {
            const data = getWorkingCapitalCycleData();
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Giorni' }, suggestedMax: 400 } },
         ...
         try {
            const data = getCashFlowProjectionData();
             const options = {
                ...
                plugins: { ...commonChartOptions.plugins, title: { display: true, text: 'Proiezioni Finanziarie 2025-2028' } }
             };
